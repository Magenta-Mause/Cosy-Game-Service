...
- **Container image — note the naming drift.** The `argo/` manifests pin `ghcr.io/magenta-mause/cosy-gameapi:v0.0.6`, but the [release workflow](.github/workflows/release.yml) derives its image name from `ghcr.io/${{ github.repository }}`, which today resolves to `ghcr.io/magenta-mause/cosy-game-service`. The names diverged when this repository was renamed from `Cosy-Gameapi`: the deployed `v0.0.4`–`v0.0.6` images *were* built by this workflow, under the old name (the `v0.0.6` image still carries `org.opencontainers.image.source: https://github.com/Magenta-Mause/Cosy-Gameapi`). The next tagged release, however, will push to `cosy-game-service` — a repository nothing deploys from. Pin the image name explicitly before cutting another release.
