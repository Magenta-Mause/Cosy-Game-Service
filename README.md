...
- **Container image — published as `cosy-gameapi`, not `cosy-game-service`.** The [release workflow](.github/workflows/release.yml) pins `ghcr.io/magenta-mause/cosy-gameapi` explicitly, matching the `argo/` manifests and the rest of the repo (crate name, compose project, container name, secret). The pin is deliberate: this repository was renamed from `Cosy-Gameapi`, so the `ghcr.io/${{ github.repository }}` the workflow used to derive would now resolve to `cosy-game-service` — an image name nothing deploys from. The deployed `v0.0.4`–`v0.0.6` images were built by this same workflow under the old name (the `v0.0.6` image still carries `org.opencontainers.image.source: https://github.com/Magenta-Mause/Cosy-Gameapi`). Keep the name pinned; do not reintroduce the context variable.
